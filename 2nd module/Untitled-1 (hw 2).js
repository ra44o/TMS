// это какая-то часть кода для дз (вроде второй пункт, хуй знает)
var student = {
    grades: [
        {name: 'math', value: 1},
...
for (let i = 0; i < firstGradeStudents.length; i++) {
    console.log(countingGradeAverage(firstGradeStudents[i].listOfSubjects));
}
...
function countingGradeAverage (arr) {
    ...
    return arr.reduce((result, { grade }) => result + grade, 0) / arr.length;

    // let AverageGrade;
    // for (let i = 0; i < arr.length; i++) {
    //     let arrayOfGradesOfOneStudent = [];
    //     for (let j =0; j < arr[i].listOfSubjects.length; j++) {
    //         arrayOfGradesOfOneStudent.push(arr[i].listOfSubjects[j].grade);
    //     }
    //     for (let m = 0; m < arrayOfGradesOfOneStudent.length; m++) {
    //         var sumOfGrades = sumOfGrades + arrayOfGradesOfOneStudent[i];
    //     }
    //     AverageGrade = sumOfGrades / arr[i].listOfSubjects.length;
    //     AverageGrade.toFixed(2);
    //     console.log(AverageGrade + ' is an average grade of ' + arr[i]);
    //     sumOfGrades === 0, AverageGrade === 0;
    // }



    //Previous way of counting of the average grade
